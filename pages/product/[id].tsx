        ...
        const similarItems = similarProducts ? similarProducts.valueSeq().map(
            (item) => {
                if (item.get('id') != product.get('id')) {
                    return (
                        <div className="mt-5 sm:ml-4 bg-white rounded-lg pt-2 pb-4 flex flex-row sm:flex-col items-center justify-center shadow-lg" key={item?.get('id')}>
                            ...
                            <div className="ml-2 -mt-2 sm:mt-2">
                                <Link href={`/product/${encodeURIComponent(item?.get('id'))}`}>
